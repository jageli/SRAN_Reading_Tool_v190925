...
def HW(node,NODE_LIST):
    ...
    # 写入列表
    NODE_LIST[checkbox_name.index("HW")].append([class1, EnodeBID, version, disName, locationName, swVersion, userLabel, vendorName])
...
def GNSSE_HW(node, NODE_LIST):
    ...
    # 写入列表
    NODE_LIST[checkbox_name.index("GNSSE_HW")].append([class1, EnodeBID, version, disName, inventoryUnitType])
...
def BBMOD_HW(node,NODE_LIST):
    ...
    # 写入列表
    NODE_LIST[checkbox_name.index("BBMOD_HW")].append([class1, EnodeBID, version, disName, inventoryUnitType, serialNumber, vendorUnitTypeNumber])
...
def SMOD_HW(node, NODE_LIST):
    ...
    # 写入列表
    NODE_LIST[checkbox_name.index("SMOD_HW")].append([class1, EnodeBID, version, disName, inventoryUnitType, serialNumber, vendorUnitTypeNumber])
...
def RMOD_HW(node, NODE_LIST):
    ...
    # 写入列表
    NODE_LIST[checkbox_name.index("RMOD_HW")].append([class1, EnodeBID, version, disName, inventoryUnitType, serialNumber, vendorName,
