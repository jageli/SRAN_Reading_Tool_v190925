...
def CARRIERGROUPC_R(node, NODE_LIST):
    ...
    # GNCELID = spliterGNCEL(disName)
    GNCELID = spliter_M(disName, 4, "GNCEL-")
    # print(GNCELID)
    ...
    # 写入列表
    NODE_LIST[checkbox_name.index("CARRIERGROUPC_R")].append([class1, EnodeBID, version, disName, GNCELID, channelGroupId, trxAdminState])
...
def GNBCF(node, NODE_LIST):
    ...
    NODE_LIST[checkbox_name.index("GNBCF")].append(
        [class1, EnodeBID, version, disName, bcfId, bscId, mPlaneRemoteIpAddressOmuSig, sctpPortOmuSig,
         txPowerPooling])
...
def GNCEL(node, NODE_LIST):
    ...
    # 写入列表
    NODE_LIST[checkbox_name.index("GNCEL")].append([class1, EnodeBID, version, disName, lcelcId, perTrxPower])

def GNCEL_R(node, NODE_LIST):
    class1 = node.attrib["class"]
    disName = node.attrib["distName"]
    EnodeBID = spliter_M(disName, 1, "MRBTS-")
    lcelcI = spliter_M(disName, 4, "GNCEL-")
    version = node.attrib["version"]

    gsmBtsId = get_child_node(node, "gsmBtsId")

    # 写入列表
    NODE_LIST[checkbox_name.index("GNCEL_R")].append([class1, EnodeBID, version, disName, lcelcI, gsmBtsId])
